...
void tampilkanMenuSorting() {
    cout << "\n=== MENU SORTING ===\n";
    cout << "1. Sortir Nama Penerima (Ascending)\n";
    cout << "2. Sortir Jumlah Bantuan (Descending)\n";
    cout << "3. Sortir Berdasarkan Jenis Bantuan\n";
...
void sortNamaAscending(Donasi dataDonasi[], int jumlahData) {
    for (int i = 0; i < jumlahData - 1; i++) {
        for (int j = 0; j < jumlahData - i - 1; j++) {
            if (dataDonasi[j].namaPenerima > dataDonasi[j + 1].namaPenerima) {
    ...
    cout << "Data telah diurutkan berdasarkan nama penerima (ascending)\n";
}

void sortJumlahDescending(Donasi dataDonasi[], int jumlahData) {
    for (int i = 0; i < jumlahData - 1; i++) {
        for (int j = 0; j < jumlahData - i - 1; j++) {
            if (dataDonasi[j].jumlahBantuan < dataDonasi[j + 1].jumlahBantuan) {
                Donasi temp = dataDonasi[j];
                dataDonasi[j] = dataDonasi[j + 1];
                dataDonasi[j + 1] = temp;
            }
    ...
    cout << "Data telah diurutkan berdasarkan jumlah bantuan (descending)\n";
}

void sortJenisBantuan(Donasi dataDonasi[], int jumlahData) {
    for (int i = 0; i < jumlahData - 1; i++) {
        for (int j = 0; j < jumlahData - i - 1; j++) {
            if (dataDonasi[j].jenisBantuan > dataDonasi[j + 1].jenisBantuan) {
                Donasi temp = dataDonasi[j];
                dataDonasi[j] = dataDonasi[j + 1];
                dataDonasi[j + 1] = temp;
            }
    ...
    cout << "Data telah diurutkan berdasarkan jenis bantuan\n";
}

void manajemenSorting(Donasi dataDonasi[], int jumlahData) {
    if (jumlahData == 0) {
    ...
    do {
        ...
        switch (pilihan) {
            case 1:
                sortNamaAscending(dataDonasi, jumlahData);
                tampilkanData(dataDonasi, jumlahData);
                break;
            case 2:
                sortJumlahDescending(dataDonasi, jumlahData);
                tampilkanData(dataDonasi, jumlahData);
                break;
            case 3:
                sortJenisBantuan(dataDonasi, jumlahData);
                tampilkanData(dataDonasi, jumlahData);
                break;
            case 4:
    ...
    do {
        ...
        switch (pilihan) {
            ...
            case 6:
                ...
                break;
            case 7:
                manajemenSorting(dataDonasi, jumlahData);
                break;
            default:
                cout << "Pilihan tidak valid\n";
        }
